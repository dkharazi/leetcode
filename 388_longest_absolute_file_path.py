...
class Solution(object):
    # https://gengwg.blogspot.com/2018/05/leetcode-388-longest-absolute-file-path.html
    def lengthLongestPath(self, input):
        ...
        """
        maxlen = 0
        pathlen = {0: 0} # initial depth 0, length 0
        # >>> "dir\n\tsubdir1\n\tsubdir2\n\t\tfile.ext".splitlines()
        # ['dir', '\tsubdir1', '\tsubdir2', '\t\tfile.ext']
        for line in input.splitlines():     # split by \n's
            name = line.lstrip('\t')        # strip left \t's
            depth = len(line) - len(name)   # depth = number of \t's
            if '.' in name: # is file
                # update max file length only
                maxlen = max(maxlen, pathlen[depth] + len(name))
            else: # is dir
                # update path length at next depth level only
