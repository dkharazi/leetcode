...
class Solution:
    ...
    def pourWater(self, heights, V, K):
        # drop one drop at K to height array
        def drop(h, K):
            best = K
            for d in (-1, 1):   # search for left first, then right
                i = K + d
                ...
                while i >=0 and i < len(h) and h[i] <= h[i - d]:
                    if h[i] < h[best]:
                        best = i
                    i += d
                if best != K:   # if find left best location, break out
                                # otherwise will find right location
                    break
            h[best] += 1
