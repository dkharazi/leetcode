...
# Line 1 - 3 is valid because it passes through key 2, 
# which had been selected in the pattern

# Valid move: 6 - 5 - 4 - 1 - 9 - 2
# Line 1 - 9 is valid because it passes through key 5, 
# which had been selected in the pattern.
...
class Solution:
    def numberOfPatterns(self, m, n):
        ...
        这道题说的是安卓机子的解锁方法，有9个数字键，
        如果密码的长度范围在[m, n]之间，问所有的解锁模式共有多少种，
        注意题目中给出的一些非法的滑动模式。
        那么我们先来看一下哪些是非法的，首先1不能直接到3，必须经过2，
        ...
        然后找他们之间是否有jump数字，
        如果next没被访问过，并且jump为0，或者jump被访问过，我们对next调用递归函数。
        ...
        visited = [False] * 10
        visited[0] = True
        hash = [[0 for _ in range(10)] for __ in range(10)]
        hash[1][3] = hash[3][1] = 2
        hash[1][7] = hash[7][1] = 4
        hash[3][9] = hash[9][3] = 6
        hash[7][9] = hash[9][7] = 8  
        ...
        return self.dfs(m, n, 1, 1, visited, hash) * 4 + self.dfs(m, n, 1, 2, visited, hash) * 4 + self.dfs(m, n, 1, 5, visited, hash)  

    def dfs(self, m, n, len, num, visited, hash):
        cnt = 0
        if len >= m:
            cnt += 1
        len += 1
        ...
        visited[num] = True
        for i in range(1, 10):
            if not visited[i] and visited[hash[num][i]]:
                cnt += self.dfs(m, n, len, i, visited, hash)
        visited[num] = False
        return cnt

print(Solution().numberOfPatterns(8, 8))